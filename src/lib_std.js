const lib_std_URLs = {
  ...
  "ecdsa_secp256k1.nr": new URL(
    "../circuits/lib/std/ecdsa_secp256k1.nr",
    import.meta.url
  ),
  ...
  "scalar_mul.nr": new URL(
    "../circuits/lib/std/scalar_mul.nr",
    import.meta.url
  ),
...
async function fetchLib() {
  ...
  const pathSourcePairs = await Promise.all(
    Object.keys(lib_std_URLs).map((currentSourceURLkey) => {
